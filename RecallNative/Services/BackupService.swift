...
struct RecallBackup: Codable {
    ...
    private enum RootKeys: String, CodingKey { case version, schemaVersion, exportedAt, decks, cards, reviews, meta, hapticsEnabled, history }

    init(version: Int, exportedAt: Date, decks: [DeckRecord], cards: [CardRecord], reviews: [ReviewRecord], history: [String:Int] = [:], hapticsEnabled: Bool? = nil) {
        self.version=version; self.exportedAt=exportedAt; self.decks=decks; self.cards=cards; self.reviews=reviews; self.history=history; self.importedHapticsEnabled=hapticsEnabled
    }
    init(from decoder: Decoder) throws {
        ...
        if let version=try root.decodeIfPresent(Int.self,forKey:.version), let exportedAt=try root.decodeIfPresent(Date.self,forKey:.exportedAt), let decks=try root.decodeIfPresent([DeckRecord].self,forKey:.decks), let cards=try root.decodeIfPresent([CardRecord].self,forKey:.cards), let reviews=try root.decodeIfPresent([ReviewRecord].self,forKey:.reviews) {
            self.init(version:version,exportedAt:exportedAt,decks:decks,cards:cards,reviews:reviews,history:try root.decodeIfPresent([String:Int].self,forKey:.history) ?? [:],hapticsEnabled:try root.decodeIfPresent(Bool.self,forKey:.hapticsEnabled)); return
        }
        ...
        self.init(version:1,exportedAt:exportedAt,decks:decks,cards:cards,reviews:[],history:meta?.history ?? [:],hapticsEnabled:meta?.hapticsEnabled)
    }
...
enum BackupService {
    static func makeBackup(context: ModelContext) throws -> Data {
        ...
        let backup=RecallBackup(version:1,exportedAt:.now,decks:decks,cards:cards,reviews:reviews,history:ReviewHistoryStore.exportValues(),hapticsEnabled:UserDefaults.standard.object(forKey:"hapticsEnabled") as? Bool ?? true)
    ...
    static func restore(_ data:Data,context:ModelContext,replaceExisting:Bool=false) throws {
        ...
        try context.save(); ReviewHistoryStore.replace(valuesFromStrings(backup.history)); if let h=backup.importedHapticsEnabled { UserDefaults.standard.set(h,forKey:"hapticsEnabled") }
    }
