...
enum BackupService {
    static func makeBackup(context: ModelContext) throws -> Data {
        let decks = try context.fetch(FetchDescriptor<Deck>()).map {
            RecallBackup.DeckRecord(id: $0.id, name: $0.name, emoji: $0.emoji, createdAt: $0.createdAt, newDay: $0.newDay, newStudiedToday: $0.newStudiedToday)
        }
    ...
    static func restore(_ data: Data, context: ModelContext, replaceExisting: Bool = false) throws {
        ...
        for record in backup.decks {
            let deck = Deck(name: record.name, emoji: record.emoji); deck.id = record.id; deck.createdAt = record.createdAt; deck.newDay = record.newDay; deck.newStudiedToday = record.newStudiedToday
    ...
    private struct ReviewLogRecord {
        let id: UUID; let reviewedAt: Date; let rating: Int; let cardID: UUID?
        init(_ log: ReviewLog) { id = log.id; reviewedAt = log.reviewedAt; rating = log.rating; cardID = log.card?.id }
    }

    enum BackupError: LocalizedError {
        case unsupportedVersion
        case invalidSize
        var errorDescription: String? { switch self { case .unsupportedVersion: return "This backup was created by an unsupported Recall version."; case .invalidSize: return "This backup is too large to import safely." } }
    }
}
