...
enum SeedDataService {
    ...
    static func insertSampleDeck(into context: ModelContext) throws {
        ...
        do {
            ...
            UsageMetricsStore.cancelSuppressedCreationDelta()
            throw error
        }
    }
    ...
    static func migrateLegacySampleDeckIfNeeded(context: ModelContext) throws {
        ...
        guard let legacy = decks.first(where: {
            $0.name.trimmingCharacters(in: .whitespacesAndNewlines) == legacySampleDeckName && matchesSampleCards($0.cards)
        }) else { return }
        legacy.name = sampleDeckName
        try context.save()
    }
    ...
    private static func matchesSampleCards(_ cards: [Flashcard]) -> Bool {
        ...
        func signature(_ q: String, _ a: String, _ h: String, _ t: String) -> String {
            "\(q)\u{1F}\(a)\u{1F}\(h)\u{1F}\(t)"
        }
        return Set(cards.map { signature($0.question, $0.answer, $0.hint, $0.tags) }) ==
            Set(sampleCards.map { signature($0.0, $0.1, $0.2, $0.3) })
    }
}
