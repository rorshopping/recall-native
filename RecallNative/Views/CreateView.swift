...
struct CreateView: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text("Turn your material into something you can study.").font(.subheadline).foregroundStyle(.secondary)
                    }
                    ...
                    RecallCard {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            Button { showingImporter = true } label: {
                                HStack(spacing: 12) {
                                    Image(systemName: "doc.richtext").font(.title2).foregroundStyle(RecallTheme.accent)
                                    VStack(alignment: .leading, spacing: 3) {
                                        Text(sourceName.isEmpty ? "Upload PDF" : sourceName).font(.headline).foregroundStyle(.primary)
                                        Text(sourceName.isEmpty ? "Up to 5 pages" : "Ready for on-device AI").font(.caption).foregroundStyle(.secondary)
                                    }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        Picker("Create as", selection: $mode) {
                            ForEach([CreateMode.flashcards, .guide, .exam]) { Text($0.rawValue).tag($0) }
                    ...
                    RecallCard {
                        VStack(alignment: .leading, spacing: 12) {
                            HStack(spacing: 10) { Image(systemName: "sparkles").foregroundStyle(RecallTheme.accent); Text("On-device AI").font(.headline); Spacer(); if modelAvailable { Image(systemName: "checkmark.circle.fill").foregroundStyle(.green) } }
                            ...
                            if isDownloadingModel {
                                ...
                                HStack { Text("Downloading"); Spacer(); Text("\(Int(downloadProgress.fraction * 100))%") }.font(.caption.weight(.medium))
                    ...
                    Button { generate() } label: { Label(isGenerating ? "Generating..." : mode == .flashcards ? "Create flashcards" : "Generate \(mode.rawValue)", systemImage: mode.icon).frame(maxWidth: .infinity) }
                        .buttonStyle(.borderedProminent).controlSize(.large).disabled(!canGenerate || isGenerating)
                    ...
                    if let resultObject, !generated.isEmpty == false { AdvancedResult(mode: mode, data: resultObject, dismiss: { self.resultObject = nil }) }
            ...
            .navigationTitle("Create").navigationBarTitleDisplayMode(.inline)
            ...
            .sheet(isPresented: $showingText) { NavigationStack { TextEditor(text: $sourceText).padding().navigationTitle("Paste notes").toolbar { ToolbarItem(placement: .cancellationAction) { Button("Cancel") { showingText = false } }; ToolbarItem(placement: .confirmationAction) { Button("Use notes") { showingText = false } } } }.presentationDetents([.large]) }
    ...
    private var canGenerate: Bool {
        guard modelAvailable else { return false }
        guard !sourceText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return false }
        if mode == .ask { return !askInput.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
        if mode == .explain { return !explainInput.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
        return true
    }
    ...
    private func downloadModel() {
        guard !isDownloadingModel else { return }; isDownloadingModel = true; downloadProgress = .init(fraction: 0, bytesWritten: 0, totalBytes: 0)
        Task { do { _ = try await modelStore.downloadModel { p in Task { @MainActor in downloadProgress = p } }; await MainActor.run { modelAvailable = true; isDownloadingModel = false } } catch { await MainActor.run { errorMessage = error.localizedDescription; isDownloadingModel = false } } }
    }

    private func handleImport(_ result: Result<[URL], Error>) {
        guard case .success(let urls) = result, let url = urls.first else { return }
        do { sourceText = try importer.extractText(from: url); sourceName = url.deletingPathExtension().lastPathComponent; mode = .flashcards } catch { errorMessage = error.localizedDescription }
    }

    private func generate() {
        guard !isGenerating else { return }; isGenerating = true; generated = []; resultObject = nil
        Task { @MainActor in
            do {
                if mode == .flashcards { generated = try await ai.generateFlashcards(from: sourceText) }
                else {
                    let object = try await advancedAI.generateJSON(instruction: instruction, systemPrompt: prompt, source: sourceText)
                    resultObject = object
                }
    ...
    private var instruction: String {
        switch mode { case .guide: return "Create a concise study guide."; case .exam: return "Create a 10-question practice exam."; case .ask: return "Answer this question: \(askInput.trimmingCharacters(in: .whitespacesAndNewlines))"; case .explain: return "Explain this concept simply: \(explainInput.trimmingCharacters(in: .whitespacesAndNewlines))"; case .flashcards: return "Create flashcards." }
    }
    private var prompt: String {
        switch mode {
        case .guide: return "Return ONLY JSON: {\"title\":\"Study guide title\",\"overview\":\"2-4 sentence overview\",\"sections\":[{\"title\":\"Section\",\"summary\":\"Summary\",\"keyPoints\":[\"Point\"],\"keyTerms\":[{\"term\":\"Term\",\"definition\":\"Definition\"}]}],\"takeaways\":[\"Takeaway\"]}. Use 3-8 sections. Every factual claim must be supported by the source. Do not invent facts."
        case .exam: return "Return ONLY JSON: {\"title\":\"Practice exam\",\"instructions\":\"Short instructions\",\"questions\":[{\"question\":\"Question\",\"type\":\"multiple_choice\",\"options\":[\"A\",\"B\",\"C\",\"D\"],\"correctIndex\":0,\"answer\":\"Answer\",\"explanation\":\"Explanation\"}]}. Create 10 questions, mixing multiple choice, true/false and short answer."
        case .ask: return "Return ONLY JSON: {\"answer\":\"Concise answer\",\"citations\":[\"Supporting fact\"]}. Use only the supplied material. If absent, say you could not find it."
        case .explain: return "Return ONLY JSON: {\"concept\":\"Concept\",\"explanation\":\"Simple explanation\",\"analogy\":\"Optional analogy\",\"keyPoints\":[\"Point\"]}. Use plain language and the supplied material."
        case .flashcards: return ""
        }
    }

    private func saveGeneratedCards() { let deck = Deck(name: deckName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "New deck" : deckName); modelContext.insert(deck); generated.forEach { modelContext.insert(Flashcard(question: $0.question, answer: $0.answer, deck: deck)) }; generated = []; sourceText = ""; sourceName = ""; deckName = ""; showingSave = false }
    private func formatBytes(_ bytes: Int64) -> String { ByteCountFormatter.string(fromByteCount: bytes, countStyle: .file) }
}

private struct FlashcardResult: View {
    let cards: [GeneratedCard]; let save: () -> Void; let dismiss: () -> Void
    var body: some View { VStack(alignment: .leading, spacing: 12) { HStack { Text("Ready to review").font(.title3.bold()); Spacer(); Text("\(cards.count) cards").font(.caption).foregroundStyle(.secondary) }; ForEach(cards) { card in RecallCard { Text(card.question).font(.headline); Divider(); Text(card.answer).foregroundStyle(.secondary) } }; Button("Save to library", systemImage: "square.and.arrow.down.fill", action: save).buttonStyle(.borderedProminent).frame(maxWidth: .infinity); Button("Discard", role: .destructive, action: dismiss).frame(maxWidth: .infinity) } }
}

private struct AdvancedResult: View {
    let mode: CreateMode; let data: [String: Any]; let dismiss: () -> Void
    var body: some View { RecallCard { VStack(alignment: .leading, spacing: 12) { Text(mode.rawValue.uppercased()).font(.caption.weight(.bold)).foregroundStyle(RecallTheme.accent).tracking(1); content; Button("Done", action: dismiss).buttonStyle(.bordered).frame(maxWidth: .infinity) } } }
    @ViewBuilder private var content: some View {
        switch mode {
        case .guide: GuideContent(data: data)
        case .exam: ExamContent(data: data)
        case .ask: Text(data["answer"] as? String ?? "No answer returned.").font(.body)
        case .explain: VStack(alignment: .leading, spacing: 8) { Text(data["concept"] as? String ?? "Explanation").font(.title3.bold()); Text(data["explanation"] as? String ?? ""); if let analogy = data["analogy"] as? String, !analogy.isEmpty { Text("Analogy").font(.headline); Text(analogy) }; ForEach((data["keyPoints"] as? [String]) ?? [], id: \.self) { Text("• \($0)") } }
        case .flashcards: EmptyView()
        }
    }
}
...
private struct ExamContent: View {
    let data: [String: Any]; @State private var revealed: Set<Int> = []
...
private struct SaveDeckSheet: View {
    @Environment(\.dismiss) private var dismiss; @Binding var name: String; let save: () -> Void
