...
struct CreateView: View {
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    RecallCard {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            if sourceName.isEmpty {
                                ...
                                TextField("Optional focus or topic", text: $focusInput, axis: .vertical).lineLimit(2...4).textFieldStyle(.roundedBorder)
                                Text("Your PDF remains the source. Add a focus to make the generated cards target a specific topic within it.").font(.caption).foregroundStyle(.secondary)
                            }
                            if !sourceName.isEmpty { Button("Remove PDF", role: .destructive) { sourceName = ""; sourceText = ""; focusInput = "" } }
                        }
                    }
                    VStack(alignment: .leading, spacing: 10) {
                        Text("Create as").font(.headline)
                        Picker("Create as", selection: $primaryMode) { Text("Flashcards").tag(CreateMode.flashcards); Text("Study guide").tag(CreateMode.guide); Text("Practice exam").tag(CreateMode.exam) }.pickerStyle(.segmented)
                        if primaryMode == .flashcards { Text("10-25 cards · recommended").font(.caption).foregroundStyle(.secondary) }
                    }
                    RecallCard {
                        VStack(alignment: .leading, spacing: 12) {
                    ...
                    if moreWaysExpanded {
                        VStack(spacing: 10) {
                            Picker("Mode", selection: $advancedMode) { Text("Ask").tag(CreateMode.ask); Text("Explain").tag(CreateMode.explain) }.pickerStyle(.segmented)
                            if advancedMode == .ask { TextField("Ask a question about your material", text: $askInput, axis: .vertical).lineLimit(2...5).textFieldStyle(.roundedBorder) }
            ...
            .navigationDestination(item: $openDeckID) { deckID in
                if let deck = decks.first(where: { $0.id == deckID }) { DeckDetailView(deck: deck) }
            }
    ...
    private var suggestedDeckName: String {
        let basis = sourceName.isEmpty ? sourceText : (focusInput.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? sourceName.replacingOccurrences(of: ".pdf", with: "", options: .caseInsensitive) : focusInput)
        let words = basis.trimmingCharacters(in: .whitespacesAndNewlines).split(separator: " ").prefix(5).joined(separator: " ")
        return words.isEmpty ? "New deck" : words.capitalized
    }
    ...
    private func handleImport(_ result: Result<[URL], Error>) { guard case .success(let urls) = result, let url = urls.first else { return }; do { sourceText = try importer.extractText(from: url); sourceName = url.deletingPathExtension().lastPathComponent + ".pdf"; focusInput = ""; primaryMode = .flashcards; moreWaysExpanded = false } catch { errorMessage = error.localizedDescription } }
    private func generationSource() -> String {
        let focus = focusInput.trimmingCharacters(in: .whitespacesAndNewlines)
...
private struct SaveDeckSheet: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Save to") {
                    if decks.isEmpty { Text("No decks yet. A new deck will be created.").foregroundStyle(.secondary) }
                    else {
                        Picker("Destination", selection: Binding(get: { selectedDeckID ?? UUID(uuidString: "00000000-0000-0000-0000-000000000000")! }, set: { value in let emptyID = UUID(uuidString: "00000000-0000-0000-0000-000000000000")!; selectedDeckID = value == emptyID ? nil : value })) {
                            ...
                            ForEach(decks) { deck in
                                ...
                                Text("\(deck.emoji)  \(deck.name) · \(isPremium ? deck.cards.count : remaining) \(isPremium ? "cards" : "slots left")").tag(deck.id).disabled(!isPremium && deck.cards.count + generatedCount > EntitlementRules.freeCardLimitPerDeck)
                            }
                        }
                    }
                }
                if selectedDeckID == nil {
                    Section("New deck") { TextField("Deck name", text: $name); if !isPremium && generatedCount > EntitlementRules.freeCardLimitPerDeck { Text("The free plan supports up to \(EntitlementRules.freeCardLimitPerDeck) cards per deck.").font(.caption).foregroundStyle(.secondary) } }
                } else if let deck = decks.first(where: { $0.id == selectedDeckID }), !isPremium {
                    Section("Capacity") { let remaining = max(0, EntitlementRules.freeCardLimitPerDeck - deck.cards.count); Label("\(remaining) card slots remaining", systemImage: remaining >= generatedCount ? "checkmark.circle" : "exclamationmark.triangle").foregroundStyle(remaining >= generatedCount ? AnyShapeStyle(.secondary) : AnyShapeStyle(Color.orange)); if remaining < generatedCount { Text("This deck does not have enough free capacity for all generated cards.").font(.caption).foregroundStyle(.secondary) } }
                }
                Section { Text("Generated flashcards are saved locally and ready for spaced repetition.").font(.subheadline).foregroundStyle(.secondary) }
            }
            .navigationTitle("Save cards")
            .toolbar { ToolbarItem(placement: .cancellationAction) { Button("Cancel") { dismiss() } }; ToolbarItem(placement: .confirmationAction) { Button("Save") { save() }.disabled(!isPremium && selectedDeckID != nil && (decks.first(where: { $0.id == selectedDeckID })?.cards.count ?? 0) + generatedCount > EntitlementRules.freeCardLimitPerDeck) } }
        }
