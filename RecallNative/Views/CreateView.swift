...
struct CreateView: View {
    ...
    private func handleImport(_ result: Result<[URL], Error>) { guard case .success(let urls) = result, let url = urls.first else { return }; do { sourceText = try importer.extractText(from: url); sourceName = url.deletingPathExtension().lastPathComponent; primaryMode = .flashcards; moreWaysExpanded = false } catch { errorMessage = error.localizedDescription } }
    private func generate() { guard !isGenerating else { return }; let mode = activeMode; isGenerating = true; generated = []; resultObject = nil; resultMode = nil; Task { @MainActor in do { if mode == .flashcards { generated = try await ai.generateFlashcards(from: sourceText) } else { let data = try await advancedAI.generateJSON(instruction: instruction(for: mode), systemPrompt: prompt(for: mode), source: sourceText); guard let object = try JSONSerialization.jsonObject(with: data) as? [String: Any] else { throw AIServiceError.generationFailed("Gemma 4 returned an invalid result.") }; resultObject = object; resultMode = mode } } catch { errorMessage = error.localizedDescription }; isGenerating = false } }
