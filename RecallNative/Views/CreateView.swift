...
struct CreateView: View {
                ...
                VStack(alignment: .leading, spacing: 18) {
                            ...
                            if isDownloadingModel {
                                VStack(alignment: .leading, spacing: 8) {
                                    ...
                                    HStack {
                                        ...
                                        Spacer()
                                        Text("\(Int(downloadProgress.fraction * 100))%")
                                    }
                                    .font(.caption.weight(.medium))
                                    HStack {
                                        Text(formatBytes(downloadProgress.bytesWritten))
                                        Text("of")
                                        Text(formatBytes(downloadProgress.totalBytes))
                                        Spacer()
                                    }
                        ...
                        RecallCard {
                            HStack(spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    Text("Creating your cards").font(.headline)
                    ...
                    if !generated.isEmpty {
                        VStack(alignment: .leading, spacing: 12) {
                            HStack {
                                Text("Ready to review").font(.title3.bold())
                                Spacer()
                                Text("\(generated.count) cards").font(.subheadline.weight(.medium)).foregroundStyle(.secondary)
                            }
    ...
    private func downloadModel() {
        ...
        Task { @MainActor in
            do {
                for try await progress in modelStore.downloadModelWithProgress() { downloadProgress = progress }
    ...
    private func handleImport(_ result: Result<[URL], Error>) {
        ...
        do { sourceText = try importer.extractText(from: url); generate() }
        catch { errorMessage = error.localizedDescription }
    }
    ...
    private func generate() {
        ...
        Task { @MainActor in
            do { generated = try await ai.generateFlashcards(from: sourceText) }
            catch { errorMessage = error.localizedDescription }
            isGenerating = false
        }
    }
...
private struct SaveDeckSheet: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Deck") { TextField("Deck name", text: $name) }
                Section { Text("Your generated cards will be saved locally and available in your library.").font(.subheadline).foregroundStyle(.secondary) }
