...
struct ReviewView: View {
    ...
    var body: some View { NavigationStack { Group { if completed { completionView } else if didInitialize && queue.isEmpty { emptyView } else if !queue.isEmpty { studyBody } else { ProgressView() } }.navigationBarTitleDisplayMode(.inline).toolbar { ToolbarItem(placement: .cancellationAction) { Button("Done") { dismiss() } } } }.task { initializeIfNeeded() } }
    private var studyBody: some View {
        let card = queue[0]
        return VStack(spacing: 18) {
            HStack { Text(deck?.name ?? "Review").font(.headline); Spacer(); Text("\(min(reviewed + 1, max(total, 1))) / \(max(total, 1))").font(.subheadline.weight(.medium)).foregroundStyle(.secondary) }
            ProgressView(value: Double(reviewed), total: Double(max(total, 1))).tint(RecallTheme.accent)
            RecallCard { VStack(alignment: .leading, spacing: 14) { HStack { Text(revealed ? "ANSWER" : "QUESTION").font(.caption.weight(.bold)).foregroundStyle(RecallTheme.accent).tracking(1); Spacer(); Text(card.state.uppercased()).font(.caption2.weight(.semibold)).foregroundStyle(.secondary) }; ScrollView { VStack(alignment: .leading, spacing: 14) { if card.mediaType == "image", let uri = card.mediaURI, let url = URL(string: uri) { AsyncImage(url: url) { image in image.resizable().scaledToFit().clipShape(RoundedRectangle(cornerRadius: 14)) } placeholder: { ProgressView() } }; Text(displayText(for: card)).font(.title2.bold()).frame(maxWidth: .infinity, alignment: .leading).textSelection(.enabled) } }; if !revealed { Text("Tap to reveal the answer").font(.subheadline).foregroundStyle(.secondary) } } }.frame(maxWidth: 760, maxHeight: .infinity).contentShape(Rectangle()).onTapGesture { withAnimation(.snappy(duration: 0.25)) { revealed = true } }
            ...
            if revealed && card.typeInAnswer { typeIn(card) } else if revealed { ratings } else { Text("How well did you remember it?").font(.caption).foregroundStyle(.secondary) }
        }.padding()
    }
    private var ratings: some View { HStack(spacing: 8) { RatingButton(title: "Again", value: 0, action: rate); RatingButton(title: "Hard", value: 1, action: rate); RatingButton(title: "Good", value: 2, action: rate); RatingButton(title: "Easy", value: 3, action: rate) }.frame(maxWidth: 760) }
    private func typeIn(_ card: Flashcard) -> some View { VStack(spacing: 10) { TextField("Type your answer", text: $typed).textFieldStyle(.roundedBorder).textInputAutocapitalization(.never).autocorrectionDisabled().onSubmit(checkTyped); if let typeChecked { Text(typeChecked ? "✓ Correct" : "✗ Answer: \(card.answer)").font(.subheadline.weight(.semibold)).foregroundStyle(typeChecked ? .green : .red) }; Button("Check", action: checkTyped).buttonStyle(.borderedProminent); if typeChecked != nil { ratings } }.frame(maxWidth: 760) }
    private var completionView: some View { VStack(spacing: 18) { Image(systemName: "checkmark.circle.fill").font(.system(size: 64)).foregroundStyle(RecallTheme.accent); Text("Session complete").font(.largeTitle.bold()); Text("\(reviewed) card\(reviewed == 1 ? "" : "s") reviewed · nice work").foregroundStyle(.secondary); Button("Study again", action: restart).buttonStyle(.borderedProminent); Button("Done") { dismiss() }.buttonStyle(.bordered) }.padding(32) }
    ...
    private func restart() { didInitialize = false; completed = false; queue = []; total = 0; reviewed = 0; revealed = false; typed = ""; typeChecked = nil; initializeIfNeeded() }
