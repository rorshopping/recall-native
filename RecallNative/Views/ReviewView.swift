...
struct ReviewView: View {
        ...
        return VStack(spacing: 18) {
            ...
            RecallCard {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 14) {
                            if card.mediaType == "image", let uri = card.mediaURI, let url = URL(string: uri) { AsyncImage(url: url) { image in image.resizable().scaledToFit().clipShape(RoundedRectangle(cornerRadius: 14)) } placeholder: { ProgressView() } }
    ...
    private func initializeIfNeeded() {
        ...
        else { let due = scoped.filter { !$0.isNew && $0.isDue }; let freshLimit = deck?.newRemainingToday ?? scoped.count; let fresh = Array(scoped.filter(\.isNew).prefix(freshLimit)); initial = due + fresh }
    ...
    private func rate(_ grade: Int) {
        ...
        let current = queue.removeFirst(); if grade == 0 { queue.append(current) }
