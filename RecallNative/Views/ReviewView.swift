...
struct ReviewView: View {
        ...
        return VStack(spacing: 18) {
            ...
            RecallCard {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 14) {
                            if card.mediaType == "image", let uri = card.mediaURI, let url = URL(string: uri) {
    ...
    private func checkTyped() {
        ...
        typeChecked = typed.trimmingCharacters(in: .whitespacesAndNewlines)
            .caseInsensitiveCompare(card.answer.trimmingCharacters(in: .whitespacesAndNewlines)) == .orderedSame
    }
