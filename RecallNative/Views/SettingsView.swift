...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Appearance") {
                    Picker("Theme", selection: $appearance) { Text("System").tag("system"); Text("Light").tag("light"); Text("Dark").tag("dark") }.pickerStyle(.menu)
                }
                Section("Study") {
    ...
    private func toggleICloud(_ enabled: Bool) {
        if enabled { let available = iCloud.isAvailable(); iCloudAvailable = available; guard available else { errorMessage = "iCloud is unavailable. Sign in to iCloud and enable the iCloud capability for this app before turning sync on."; return }; iCloudEnabled = true; syncNow() }
        else { iCloudEnabled = false }
    }
    private func syncNow() { guard iCloudEnabled else { return }; do { if try iCloud.push(context: modelContext) { iCloud.markSynced(); lastSync = iCloud.lastSyncDate() } else { errorMessage = "iCloud is unavailable on this device." } } catch { errorMessage = error.localizedDescription } }
