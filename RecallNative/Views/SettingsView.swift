...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("On-device AI") {
                    Button { showingAIInfo = true } label: {
                        HStack {
                            Label(aiProvider.displayName, systemImage: aiProvider.systemImage)
                            Spacer()
                            Image(systemName: aiProvider.isAvailable ? "checkmark.circle.fill" : "chevron.right")
                                .foregroundStyle(aiProvider.isAvailable ? .green : .tertiary)
                        }
                    }
                    Text(aiProvider.detail).font(.caption).foregroundStyle(.secondary)
                    ...
                    Text("Generation runs locally. Apple’s on-device model is preferred when available, with Gemma 4 E2B as the automatic fallback. You can also import a deck generated by another AI using Recall's JSON format.").font(.caption).foregroundStyle(.secondary)
                }
                Section("About") {
            ...
            .task { biometricLock.refreshAvailability(); iCloudAvailable = iCloud.isAvailable(); lastSync = iCloud.lastSyncDate(); iCloudState = iCloud.state(); modelAvailable = await modelStore.modelURL() != nil; await subscriptions.load() }
