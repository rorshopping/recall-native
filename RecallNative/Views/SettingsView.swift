...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                Section("Security") {
                    if biometricLock.available {
                        Toggle(isOn: Binding(
                            ...
                            set: { value in Task { await toggleBiometricLock(value) } }
                        )) {
            ...
            } message: { Text("This replaces all current decks, cards, and review history. This cannot be undone.") }
            .confirmationDialog("Restore from iCloud?", isPresented: $showingICloudRestoreConfirmation, titleVisibility: .visible) {
                Button("Restore and Replace", role: .destructive) { restoreFromICloud() }
                Button("Cancel", role: .cancel) {}
            } message: { Text("This replaces all current local decks, cards, and review history with the latest iCloud backup. This cannot be undone.") }
