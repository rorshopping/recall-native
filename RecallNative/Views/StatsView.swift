            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                HStack(spacing: 10) {
                                    ...
                                    VStack(alignment: .leading) {
                                        Text("\(streak)").font(.title.bold())
                                        Text("day streak").font(.caption).foregroundStyle(.secondary)
                                    }
                                }
                                Text(streakAtRisk ? "Study today to keep it 🔥" : "\(cards.filter(\.isNew).count) new today")
                                    .font(.caption.weight(.semibold)).foregroundStyle(streakAtRisk ? .orange : .secondary)
                            }
                            Spacer(minLength: 0)
                        }
                    }
                    ...
                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                        StatCard(title: "Due today", value: "\(cards.filter { !$0.isNew && $0.isDue }.count)", icon: "clock.fill")
    ...
    var body: some View {
        HStack(alignment: .bottom, spacing: 8) {
            ForEach(Array(values.enumerated()), id: \.offset) { _, item in
                VStack(spacing: 6) {
                    Spacer(minLength: 0)
                    RoundedRectangle(cornerRadius: 6).fill(item.today ? RecallTheme.accent : .secondary.opacity(item.count > 0 ? 0.5 : 0.12))
                        .frame(height: max(5, CGFloat(item.count) / CGFloat(maxValue) * 90))
