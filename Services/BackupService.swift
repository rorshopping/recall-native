...
struct RecallBackup: Codable {
    struct DeckRecord: Codable {
        let id: UUID; let name: String; let emoji: String; let createdAt: Date
        let newLimit: Int?; let newDay: String?; let newStudiedToday: Int?
    }
    struct CardRecord: Codable {
        let id: UUID; let question: String; let answer: String; let hint: String?; let tags: String?
        let cardType: String?; let typeInAnswer: Bool?; let mediaType: String?; let mediaURL: String?
        let createdAt: Date; let dueAt: Date; let interval: Int; let ease: Double; let repetitions: Int
        let state: String; let step: Int; let lapses: Int; let lastReviewedAt: Date?; let deckID: UUID
    }
...
enum BackupService {
    static func makeBackup(context: ModelContext) throws -> Data {
        ...
        let cards = try context.fetch(FetchDescriptor<Flashcard>()).compactMap { card -> RecallBackup.CardRecord? in
            ...
            return .init(id: card.id, question: card.question, answer: card.answer, hint: card.hint, tags: card.tags, cardType: card.cardType, typeInAnswer: card.typeInAnswer, mediaType: card.mediaType, mediaURL: card.mediaURL, createdAt: card.createdAt, dueAt: card.dueAt, interval: card.interval, ease: card.ease, repetitions: card.repetitions, state: card.state, step: card.step, lapses: card.lapses, lastReviewedAt: card.lastReviewedAt, deckID: deckID)
        }
    ...
    static func restore(_ data: Data, context: ModelContext, replaceExisting: Bool = false) throws {
        ...
        for record in backup.decks {
            let deck = Deck(name: record.name, emoji: record.emoji, newLimit: record.newLimit ?? 20)
            deck.id = record.id; deck.createdAt = record.createdAt; deck.newDay = record.newDay; deck.newStudiedToday = record.newStudiedToday ?? 0
        ...
        for record in backup.cards where deckMap[record.deckID] != nil {
            let card = Flashcard(question: record.question, answer: record.answer, deck: deckMap[record.deckID], hint: record.hint ?? "", tags: record.tags ?? "", cardType: record.cardType ?? "basic", typeInAnswer: record.typeInAnswer ?? false, mediaType: record.mediaType, mediaURL: record.mediaURL)
            card.id = record.id; card.createdAt = record.createdAt; card.dueAt = record.dueAt; card.interval = record.interval; card.ease = record.ease; card.repetitions = record.repetitions
