...
struct ReviewView: View {
    ...
    private var initialCandidates: [Flashcard] {
        ...
        let due = scopedCards.filter { !$0.isNew && $0.dueAt <= .now }
    ...
    var body: some View {
        NavigationStack {
            ...
            .task { if queueIDs.isEmpty && !completed { queueIDs = initialCandidates.map(\.id) } }
        }
    }

    private func studyCard(_ card: Flashcard) -> some View {
        VStack(spacing: 18) {
            HStack { Text(deck?.name ?? "Review").font(.headline); Spacer(); Text("\(reviewed + 1) / \(max(reviewed + queueIDs.count, 1))").font(.subheadline.weight(.medium)).foregroundStyle(.secondary) }
            ...
            RecallCard {
                ScrollView {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        Text(card.clozeText(revealed: revealed)).font(.title2.bold()).frame(maxWidth: .infinity, alignment: .leading)
                        if revealed && !card.hint.isEmpty { Text("Hint: \(card.hint)").font(.subheadline).foregroundStyle(.secondary) }
    ...
    private func typeIn(_ card: Flashcard) -> some View {
        VStack(spacing: 10) {
            TextField("Type your answer", text: $typed).textFieldStyle(.roundedBorder).textInputAutocapitalization(.never).autocorrectionDisabled()
    ...
    private var completionView: some View {
        VStack(spacing: 18) {
            ...
            Button("Study again") { queueIDs = initialCandidates.map(\.id); reviewed = 0; completed = false; revealed = false; typed = ""; typeChecked = nil }.buttonStyle(.borderedProminent)
    ...
    private var emptyView: some View {
        VStack(spacing: 14) {
            ...
            if !studyAll && !scopedCards.isEmpty { Button("Study all \(scopedCards.count) cards") { queueIDs = scopedCards.map(\.id); reviewed = 0; completed = false }.buttonStyle(.borderedProminent) }
            Button("Done") { dismiss() }.buttonStyle(.bordered)
        }.padding(32)
    }
    ...
    private func rate(_ grade: Int) {
        ...
        queueIDs.removeFirst(); if grade == 0 { queueIDs.append(card.id) }
